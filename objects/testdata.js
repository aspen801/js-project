export const lesson = {
    name: 'ДеструКТУРИЗАЦИЯ',
    description: 'уроК пО дестРуктуРиЗации',
  };

  export const company1 = { name: 'Hexlet', state: 'moderating', website: 'https://hexlet.io' };
  export const company2 = { name: 'CodeBasics', state: 'published', website: 'https://code-basics.com' };

  export const company3 = { name: 'Hexlet', state: 'published', website: 'https://hexlet.io' };
  export const company4 = { name: 'Hexlet', state: 'published', website: 'https://hexlet.io' };

  export var domain = 'http://github.com/aspen801/js-project/blob/main/domain.js';
  export var loremText = 'lorem LOrEm  ipsum   forem aren    gobel    IpSuM   gObel     GobEl';

  export const testObject = {
    user: 'ubuntu',
    hosts: {
      zero: {
        name: 'web1',
      },
      1: {
        name: 'web2',
        null: 3,
        active: false,
      },
    },
  };

  export const site = {
    name: null,
    state: 'moderating',
    car: 'Nissan'
  };
  
  export const data = {
    name: 'Hexlet',
    state: 'published',
    voice: 'high',
    car: 'Chevrolet'
  };